...
TOKEER_DIR = dirname(abspath(__file__))
...
DEFAULT_DECOMPILER_DL = join(TOKEER_DIR, 'logic/main.dl')
"""Decompiler specification file."""

FALLBACK_DECOMPILER_DL = join(TOKEER_DIR, 'logic/alt.dl')
"""Fallback decompiler specification file, optimized for scalability."""

DEFAULT_INLINER_DL = join(TOKEER_DIR, 'clientlib/function_inliner.dl')
"""IR helping inliner specification file."""

# TOKEER_DL = join(TOKEER_DIR, 'logic/external_call.dl')

DEFAULT_INLINER_ROUNDS = 6

DEFAULT_CACHE_DIR = join(TOKEER_DIR, 'cache')
...
functor_path = join(TOKEER_DIR, 'souffle-addon')
...
if not os.path.isfile(join(functor_path, 'libfunctors.so')):
    raise Exception(
        ...
        f'that you have installed tokeer correctly (see README.md)'
    )
...
def get_souffle_macros():
    souffle_macros = f'TOKEER_DIR={TOKEER_DIR} BULK_ANALYSIS= {args.souffle_macros}'.strip()
...
def analyze_contract(job_index: int, index: int, contract_filename: str, result_queue, timeout) -> None:   
    ...
    try:
        ...
        # run_clients([TOKEER_DL], [], out_dir, out_dir)
        ...
        get_tokeer_analytics(out_dir, analytics)
...
def get_tokeer_analytics(out_dir, analytics):
    for fname in os.listdir(out_dir):
        fpath = join(out_dir, fname)
...
# compile_processes_args.append((TOKEER_DL, TOKEER_DL+SOUFFLE_COMPILED_SUFFIX))
